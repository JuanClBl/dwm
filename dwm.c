	...
	*lastbutton = '0' + ev->button;
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		/* 2px right padding */
		else if (ev->x > (x = selmon->ww - (int)TEXTW(stext) - getsystraywidth() + lrpad - 2)) {
			click = ClkStatusText;
	...
	if ((w = m->ww - sw - stw - x) > bh) {
		if (n > 0) {
			for (c = m->clients; c; c = c->next) {
				...
				drw_text(drw, x, 0, (1.0 / (double)n) * w, bh, lrpad / 2 + (c->icon ? c->icw + ICONSPACING : 0), c->name, 0);
				if (c->icon) drw_pic(drw, x + lrpad / 2, (bh - c->ich) / 2, c->icw, m->sel->ich, c->icon);
				x += (1.0 / (double)n) * w;
			}
		} else {
			drw_setscheme(drw, scheme[SchemeNorm]);
